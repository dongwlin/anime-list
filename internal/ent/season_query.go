...
func (sq *SeasonQuery) QueryAnime() *AnimeQuery {
	query := (&AnimeClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SeasonQuery) WithAnime(opts ...func(*AnimeQuery)) *SeasonQuery {
	query := (&AnimeClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *SeasonQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Season, error) {
	...
	if query := sq.withAnime; query != nil {
		if err := sq.loadAnime(ctx, query, nodes, nil,
			func(n *Season, e *Anime) { n.Edges.Anime = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SeasonQuery) loadEpisodes(ctx context.Context, query *EpisodeQuery, nodes []*Season, init func(*Season), assign func(*Season, *Episode)) error {
	...
	for _, n := range neighbors {
		fk := n.season_episodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "season_episodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "season_episodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (sq *SeasonQuery) loadAnime(ctx context.Context, query *AnimeQuery, nodes []*Season, init func(*Season), assign func(*Season, *Anime)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Season)
	for i := range nodes {
		if nodes[i].anime_seasons == nil {
			continue
		}
		fk := *nodes[i].anime_seasons
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "anime_seasons" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
