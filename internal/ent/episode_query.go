...
func (eq *EpisodeQuery) loadSeason(ctx context.Context, query *SeasonQuery, nodes []*Episode, init func(*Episode), assign func(*Episode, *Season)) error {
	...
	for i := range nodes {
		if nodes[i].season_episodes == nil {
			continue
		}
		fk := *nodes[i].season_episodes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "season_episodes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
